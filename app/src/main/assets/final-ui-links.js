...
function install(){
  document.addEventListener('click',function(e){
    ...
    if(span){
      ...
      if(t.indexOf('view all')>=0){if(typeof window.toast==='function')window.toast('More content will open here.');return;}
    ...
    if(tab){var name=((tab.querySelector('b')||{}).textContent||'').trim();if(name==='Skills'&&typeof window.ssLearnSkills==='function'){window.ssLearnSkills();return;}if(name==='Academic'&&typeof window.go==='function'){window.go('learn');return;}if(name==='Other'&&typeof window.toast==='function'){window.toast('Other learning content is coming next.');return;}}
    var subject=target.closest('.ss-subject,.ss-topic');
    if(subject&&typeof window.toast==='function'){window.toast('This learning section is ready for content integration.');return;}
  ...
  document.querySelectorAll('.ss-section span').forEach(function(span){span.style.cursor='pointer';span.style.pointerEvents='auto';span.style.userSelect='none';if(!span.getAttribute('role'))span.setAttribute('role','button');});
}
