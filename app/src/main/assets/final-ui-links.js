  ...
  document.addEventListener('click',function(e){
    ...
    if(span){
      ...
      if(t.indexOf('view all')>=0){
        if(typeof window.toast==='function')window.toast('More content will open here.');
        return;
      }
      if(t.indexOf('change class')>=0){
        if(typeof window.toast==='function')window.toast('Choose a class below.');
        return;
      }
      if(t.indexOf('choose')>=0||t.indexOf('select')>=0){
        if(typeof window.toast==='function')window.toast('Choose an option below to continue.');
        return;
      }
    }
