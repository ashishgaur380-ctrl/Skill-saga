...
async function ssScheduledQuizzes(){
  var today=new Date().toISOString().slice(0,10),out={daily:null,weekly:null};
  if(typeof cloudDb==='undefined'||!cloudDb)return out;
  try{
    var snap=await cloudDb.collection('quizzes').where('published','==',true).get();
    var arr=snap.docs.map(function(d){return Object.assign({id:d.id},d.data())}).filter(function(x){
      var pd=x.publishDate||x.date||'';
      return !pd||pd<=today;
    });
    arr.sort(function(a,b){return String(b.publishDate||b.date||'').localeCompare(String(a.publishDate||a.date||''));});
    out.daily=arr.find(function(x){return String(x.type||'').toLowerCase()==='daily';})||null;
    out.weekly=arr.find(function(x){return String(x.type||'').toLowerCase()==='weekly';})||null;
  }catch(e){console.warn('Scheduled quiz load failed',e)}
  return out;
}
function ssQuizMeta(q){
  if(!q)return '';
  var n=Array.isArray(q.questions)?q.questions.length:Number(q.questionCount||q.totalQuestions||0);
  var d=q.difficulty||q.level||'';
  return (n?n+' Questions':'Quiz')+(d?' • '+d:'');
}
async function ssOpenScheduledQuiz(id){
  if(!id)return toast('This quiz is not currently available.');
  if(typeof startQuiz==='function')return startQuiz(id);
  toast('Quiz engine is not available.');
}
async function ssUpdateHomeScheduled(){
  var el=document.getElementById('ssDailyCard'),section=document.getElementById('ssWeeklySection'),w=document.getElementById('ssWeeklyCard');
  if(!el)return;
  var q=await ssScheduledQuizzes();
  el.innerHTML=q.daily?
    '<div style="font-size:9px;font-weight:900">DAILY QUIZ</div><div style="font-size:19px;font-weight:1000;margin:7px 0">'+esc(q.daily.title||'Daily Quiz')+'</div><div style="font-size:10px">'+esc(ssQuizMeta(q.daily))+'</div><button class="ss-action" style="background:#fff;color:#1769ff" onclick="ssOpenScheduledQuiz(\''+esc(q.daily.id)+'\')">Start Daily Quiz →</button>':
    '<div style="font-size:16px;font-weight:1000">Daily Quiz</div><div style="font-size:10px;margin-top:5px">No daily quiz is currently published.</div>';
  el.style.background=q.daily?'linear-gradient(135deg,#1769ff,#6544ec)':'';
  el.style.color=q.daily?'#fff':'';
  if(section&&w){
    w.innerHTML=q.weekly?
      '<div style="font-size:9px;font-weight:900;color:#1769ff">WEEKLY QUIZ</div><div style="font-size:19px;font-weight:1000;margin:7px 0">'+esc(q.weekly.title||'Weekly Quiz')+'</div><div class="muted" style="font-size:10px">'+esc(ssQuizMeta(q.weekly))+'</div><button class="ss-action" style="margin-top:10px" onclick="ssOpenScheduledQuiz(\''+esc(q.weekly.id)+'\')">Start Weekly Quiz →</button>':
      '<b>Weekly Quiz</b><div class="muted" style="margin-top:5px">No weekly quiz is currently published.</div>';
  }
}
async function ssUpdatePlayScheduled(){
  var el=document.getElementById('ssScheduledPlay');if(!el)return;
  var q=await ssScheduledQuizzes();
  var html='';
  if(q.daily)html+='<div class="ss-assignment"><div class="i">📅</div><div class="ss-assignment-main"><b>'+esc(q.daily.title||'Daily Quiz')+'</b><small>'+esc(ssQuizMeta(q.daily))+' • Daily</small></div><button onclick="ssOpenScheduledQuiz(\''+esc(q.daily.id)+'\')">Open</button></div>';
  if(q.weekly)html+='<div class="ss-assignment"><div class="i">📆</div><div class="ss-assignment-main"><b>'+esc(q.weekly.title||'Weekly Quiz')+'</b><small>'+esc(ssQuizMeta(q.weekly))+' • Weekly</small></div><button onclick="ssOpenScheduledQuiz(\''+esc(q.weekly.id)+'\')">Open</button></div>';
  if(!html)html='<div class="ss-card"><b>No scheduled quizzes</b><div class="muted">Daily and weekly quizzes published by Admin will appear here.</div></div>';
  el.innerHTML=html;
}
function homeFinal(){var s=stats(),u=U()||{},cs=competitionStats(u),rankLabel=s.xp>0?'#1':'—';base(`<div class="ss-final"><section class="ss-hero"><div class="ss-hero-copy"><div class="ss-eyebrow">LEARN • PLAY • COMPETE • GROW</div><h1 class="ss-title">Small Steps<br><span style="color:#1769ff">Big Achievements!</span></h1><div class="ss-sub">Explore. Practice. Compete. Build a brighter tomorrow.</div><div class="ss-quote">“Play • Learn • Win!”</div></div><div class="ss-hero-art"><div class="ss-hero-circle"></div><div class="ss-hero-words">Play<br><b>Learn</b><br>Win<i></i></div><div class="ss-hero-mascot">🎓</div></div></section><div class="ss-blue-stats"><div class="ss-stat"><div class="ss-stat-icon">⭐</div><b>${s.xp.toLocaleString()}</b><small>XP Points</small></div><div class="ss-stat"><div class="ss-stat-icon">🔥</div><b>${s.streak}</b><small>Day Streak</small></div><div class="ss-stat"><div class="ss-stat-icon">🏆</div><b>${rankLabel}</b><small>Current Rank</small></div></div><div class="ss-section"><b>Today's Mission</b><span onclick="window.go('play')">View all →</span></div><div id="ssDailyCard" class="ss-card ss-blue-stats" style="display:block;background:linear-gradient(135deg,#1769ff,#6544ec);color:#fff;padding:16px"></div><div class="ss-section" id="ssWeeklySection"><b>Weekly Quiz</b><span onclick="window.go('play')">Open in Play →</span></div><div id="ssWeeklyCard" class="ss-card" style="padding:16px;margin-bottom:10px"></div><div class="ss-section"><b>Your Skills</b><span onclick="window.go('learn')">View all →</span></div><div class="ss-cards">${[['🔢','Numerical',skillPct(u,'Numerical')+'%','ss-soft'],['🧪','Scientific Thinking',skillPct(u,'Scientific Thinking')+'%','ss-green'],['📖','Vocabulary',skillPct(u,'Vocabulary')+'%','ss-pink'],['🧠','Reasoning',skillPct(u,'Reasoning')+'%','ss-purple']].map(function(x){return '<div class="ss-card '+x[3]+' center"><div class="i">'+x[0]+'</div><b>'+x[1]+'</b><small style="font-weight:1000;color:#172542">'+x[2]+'</small></div>'}).join('')}</div><div class="ss-section"><b>Next Milestone</b><span>${s.xp%500}/500 XP</span></div><div class="ss-progress-card"><div style="display:flex;justify-content:space-between;font-size:10px;font-weight:900"><b>Level ${s.level}</b><b>${s.xp%500}/500 XP</b></div><div class="ss-bar"><i style="width:${Math.min(100,(s.xp%500)/5)}%"></i></div><small style="display:block;color:#78869d;font-size:8px;margin-top:7px">Keep learning to reach your next level.</small></div></div>`, 'home')}
...
window.home=homeFinal;window.play=playFinal;window.compete=competeFinal;window.notifications=ssLiveNotifications;window.skills=learnSkills;window.ssLearnSkills=learnSkills;window.ssClass=ssClass;window.ssPlayAction=ssPlayAction;window.ssOpenScheduledQuiz=ssOpenScheduledQuiz;window.ssForum=forum;
